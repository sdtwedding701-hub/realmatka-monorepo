...
const pannaEntries = Object.entries(pannaChartBySingle);
const sattaEntries = Object.entries(sattaCardBySingle);

export const allTriplePannas = ["000", "111", "222", "333", "444", "555", "666", "777", "888", "999"];
export const allCardPannas = [...new Set(sattaEntries.flatMap(([, pannas]) => pannas))].sort();
...
const pannaSingleLookup = new Map();
const sattaCardLookup = new Map();

for (const [single, pannas] of pannaEntries) {
  for (const panna of pannas) {
    pannaSingleLookup.set(panna, single);
  }
}

for (const [single, pannas] of sattaEntries) {
  for (const panna of pannas) {
    sattaCardLookup.set(panna, single);
  }
}

export function getPannaSingleDigit(panna) {
  if (allTriplePannas.includes(panna)) {
    return panna[0] ?? null;
  }
  return pannaSingleLookup.get(panna) ?? null;
}

export function getSattaCardDigit(panna) {
  return sattaCardLookup.get(panna) ?? null;
}

export function getPannaType(panna) {
  if (!/^\d{3}$/.test(panna)) {
    return "unknown";
  }
  ...
  if (uniqueDigits === 1 && allTriplePannas.includes(panna)) {
    return "triple";
  }
  if (uniqueDigits === 2) {
    return "double";
...
export function isValidPanna(panna) {
